...
| `splits.py` | Split **temporel** train/val/test chronologique (anti-fuite : train < val < test). |
...
| `registry.py` (+ table `model_registry`) | Registre **JSON canonique** + miroir DB ; modèle **actif** par `(cible, horizon)`. |
...
python -m pytest -q           # 88 tests verts (66 Phase 1 + 22 Phase 2)
```
